...
export class ApiService {
    ...
    get<T>(url: string, options?: ApiRequestOptions): Observable<T> {
        return this.http.get<T>(url, options).pipe(catchError((err) => this.handleError(err)));
    }

    post<T>(url: string, body: unknown, options?: ApiRequestOptions): Observable<T> {
        return this.http.post<T>(url, body, options).pipe(catchError((err) => this.handleError(err)));
    }

    put<T>(url: string, body: unknown, options?: ApiRequestOptions): Observable<T> {
        return this.http.put<T>(url, body, options).pipe(catchError((err) => this.handleError(err)));
    }

    patch<T>(url: string, body: unknown, options?: ApiRequestOptions): Observable<T> {
        return this.http.patch<T>(url, body, options).pipe(catchError((err) => this.handleError(err)));
    }

    delete<T>(url: string, options?: ApiRequestOptions): Observable<T> {
        return this.http.delete<T>(url, options).pipe(catchError((err) => this.handleError(err)));
    }
